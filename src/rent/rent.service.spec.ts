...
import { Rate, Book_session } from "../entities";
...
describe("Rent Service", () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(Book_session),
          useValue: DBStub
        }
      ]
    }).compile();
    serviceProvider = module.get(RentService);
    bookSessionRepository = module.get(getRepositoryToken(Book_session));
  });

  afterEach(() => {
    DBStub.query.mockClear();
    DBStub.createQueryBuilder.mockClear();
  ...
  describe("IsAvailbale Tests", () => {
    it("should return true with a warning if dates are available but period too long", async () => {
      ...
      bookSessionRepository.createQueryBuilder.getCount.mockResolvedValueOnce(0);
      const result = await serviceProvider.isAvailable(input);

      expect(bookSessionRepository.createQueryBuilder).toHaveBeenCalledWith("book_session");
    ...
    //     end: new Date("2022-08-31")
    //   };
    //   DBStub.createQueryBuilder().getCount.mockResolvedValueOnce(1);
    //   const result = await serviceProvider.isAvailable(input);
